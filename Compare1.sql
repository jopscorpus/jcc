SET @SQL = 
DELETE FROM InterSystemDI_SMDB.dbo.Fact_DI_EI3_FBI    
WHERE    
  id_day = ' + @ID_CurrectDay + '     
 AND id_interface IN (SELECT id_interface FROM InterSystemDI_ETL.dbo.DimInterfaces WHERE Interface_Name = ''FBI CFR'');    
